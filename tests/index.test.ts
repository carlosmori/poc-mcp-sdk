...
import { APIPromise } from 'mcp_poc_stainless/core/api-promise';

import util from 'node:util';
import McpPocStainless from 'mcp_poc_stainless';
import { APIUserAbortError } from 'mcp_poc_stainless';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['MCP_POC_STAINLESS_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: McpPocStainless) => {
    ...
    test('default logLevel is warn', async () => {
      const client = new McpPocStainless({ apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['MCP_POC_STAINLESS_LOG'] = 'debug';
      const client = new McpPocStainless({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['MCP_POC_STAINLESS_LOG'] = 'not a log level';
      const client = new McpPocStainless({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'MCP_POC_STAINLESS_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['MCP_POC_STAINLESS_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new McpPocStainless({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['MCP_POC_STAINLESS_BASE_URL'] = 'https://example.com/from_env';
      const client = new McpPocStainless({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['MCP_POC_STAINLESS_BASE_URL'] = ''; // empty
      const client = new McpPocStainless({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://msie-profiles-por-fire.trycloudflare.com');
    });

    test('blank env variable', () => {
      process.env['MCP_POC_STAINLESS_BASE_URL'] = '  '; // blank
      const client = new McpPocStainless({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://msie-profiles-por-fire.trycloudflare.com');
    });
    ...
    test('env variable with environment', () => {
      ...
      expect(
        () => new McpPocStainless({ apiKey: 'My API Key', environment: 'production' }),
      ).toThrowErrorMatchingInlineSnapshot(
        `"Ambiguous URL; The \`baseURL\` option (or MCP_POC_STAINLESS_BASE_URL env var) and the \`environment\` option are given. If you want to use the environment you must pass baseURL: null"`,
      );
    ...
    test('in request options overridden by client options', () => {
      const client = new McpPocStainless({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['MCP_POC_STAINLESS_BASE_URL'] = 'http://localhost:5000/env';
      const client = new McpPocStainless({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new McpPocStainless({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new McpPocStainless({ apiKey: 'My API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['MCP_POC_STAINLESS_API_KEY'] = 'My API Key';
    const client = new McpPocStainless();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['MCP_POC_STAINLESS_API_KEY'] = 'another My API Key';
    const client = new McpPocStainless({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new McpPocStainless({ apiKey: 'My API Key', fetch: testFetch });
