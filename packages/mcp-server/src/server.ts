...
export async function initMcpServer(params: {
  ...
  let _client: McpPocStainless | undefined;
  let _clientError: Error | undefined;
  let _logLevel: 'debug' | 'info' | 'warn' | 'error' | 'off' | undefined;

  const getClient = (): McpPocStainless => {
    if (_clientError) throw _clientError;
    if (!_client) {
      try {
        _client = new McpPocStainless({
          ...{ environment: (readEnv('MCP_POC_STAINLESS_ENVIRONMENT') || undefined) as any },
          logger,
