...
import { McpPocStainless, ClientOptions } from 'mcp_poc_stainless';
...
function getTSDiagnostics(code: string): string[] {
  ...
  const codeWithImport = [
    ...
    functionSource.type === 'declaration' ?
      `async function run(${functionSource.client}: McpPocStainless)`
    : `const run: (${functionSource.client}: McpPocStainless) => Promise<unknown> =`,
    functionSource.code,
  ].join('\n');
...
const fuse = new Fuse(['client.status.check', 'client.org.listRegions', 'client.internal.createAPIKey'], {
  threshold: 1,
  shouldSort: true,
});
